...
RUN apt-get update \
  && apt-get install -y --no-install-recommends bash ca-certificates git jq patch procps ripgrep tini \
...
RUN chmod 0555 /opt/harness/harness-worker.mjs /opt/harness/clone-helper.sh /opt/harness/task-runner.sh /opt/harness/shell-runner.sh /opt/harness/worker-runner.sh \
