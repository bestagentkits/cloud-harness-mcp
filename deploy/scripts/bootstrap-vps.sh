...
# Operator-provided agent skills for the executor's read-only `built-in` tier.
install -d -m 0755 /var/lib/cloud-harness/skills
...
if [[ ! -f /etc/cloud-harness-mcp/runtime.env ]]; then
  ...
  install -m 0600 /dev/null /etc/cloud-harness-mcp/runtime.env
  {
    ...
    printf '%s\n' 'EXECUTOR_IMAGE=cloud-harness-executor:local' 'NETWORK_GUARD_IMAGE=cloud-harness-network-guard:local' 'ALLOWED_GIT_HOSTS=github.com' 'WORKSPACE_NETWORK_PROFILE=dependency-access'
