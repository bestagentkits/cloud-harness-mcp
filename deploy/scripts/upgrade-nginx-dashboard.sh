...
gateway_count=$(grep -Fxc '    location = /mcp-gateway {' "$block_file" || true)
...
gateway_block=$(sed -n '/^    location = \/mcp-gateway {$/,/^    }$/p' "$block_file")
normalize_block() {
  sed -E \
    -e 's/[[:space:]]*#.*$//' \
    -e 's/^[[:space:]]+//' \
...
gateway_installed=0
if [[ $gateway_count -eq 1 && $(normalize_block <<< "$gateway_block") == "$expected_gateway" ]]; then
  gateway_installed=1
fi

if [[ $dashboard_installed -eq 1 && $api_key_installed -eq 1 && $gateway_installed -eq 1 ]]; then
  "$nginx_command" -t
...
if [[ $gateway_installed -eq 0 ]] &&
   { [[ $gateway_count -ne 0 ]] || grep -Fq '127.0.0.1:3100/mcp-gateway' "$block_file"; }; then
  echo "existing gateway nginx routing is not the managed shape; refusing to overwrite it" >&2
  exit 8
fi
...
awk -v closing="$end_line" -v add_dashboard="$((1 - dashboard_installed))" -v add_api_key="$((1 - api_key_installed))" -v add_gateway="$((1 - gateway_installed))" '
  NR == closing {
    if (add_gateway) {
      ...
      print "        proxy_pass http://127.0.0.1:3100/mcp-gateway;"
...
trap 'rm -f -- "$block_file" "$rendered"' EXIT
echo "installed Cloud Harness dashboard, API-key, and gateway nginx routes; backup: $backup"
