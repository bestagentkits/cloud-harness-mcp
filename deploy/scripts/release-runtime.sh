...
record_images() {
  ...
  docker image inspect cloud-harness-agent:local --format '{{.Id}}' > "$state/${prefix}-agent-image"
  docker image inspect cloud-harness-model-gateway:local --format '{{.Id}}' > "$state/${prefix}-model-gateway-image"
}
