...
| `BUILTIN_SKILLS_ROOT` | `/var/lib/cloud-harness/skills` | Optional | Operator-provided agent skills. Set to a host directory (absolute) and it is mounted read-only into every executor at /opt/cloud-harness/skills, the worker's highest-precedence `built-in` skills tier, and rescanned by the runner from this same directory to attribute that partition. Content is operator-owned: the harness never writes to it and records its provenance as built-in. CH_BUILTIN_SKILLS_ROOT is a separate in-executor path override, never a runner host path. Leave unset to keep the tier empty. |
