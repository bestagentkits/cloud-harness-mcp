...
- Not yet done: the remaining phase 1 work (the `StateStore` CRUD helpers, `listSkillUsage`, the generation-fenced update, and the internal-runner-api skill operations).

### Verification Results (phases 2-9 re-verification) — 2026-09-20
- Scope: after the rebase, every repository claim in phases 2 through 9 was re-checked against the working tree. Three read-only subagents covered phases 2-3, 6-7, and 8-9; the phases 4-5 child died on a provider 503, so the controller verified those anchors directly. Evidence and verdicts are retained in the subagent artifacts and summarised here.
- Load-bearing findings that would have caused a wrong implementation, all now corrected in the phase files:
  - The toolkit CAS root is `TOOLKIT_CACHE_ROOT` (default `/var/lib/cloud-harness/cache/toolkits`, `packages/contracts/src/config.ts:219`, bundle at `<root>/<ownerId>/<bundleSha256>` per `toolkit-cache-manager.ts:39`, mounted at `compose.yaml:88`); `/opt/cloud-harness/cache/...` exists nowhere. Phases 2 and 3 cited the non-existent path, and phase 3's mount guard protected a path that cannot be mounted.
  - `toolkitNetworkPolicy` is at `config.ts:220` (not `:196`) and three acquisition paths throw under `cache-only`; the egress-authority wording is at `docs/security-model.md:466` (not `:379-380`).
  - The shipped Git adapter already caps at 64 MiB and 1,000 files (`mattpocock-adapter.ts:70`), so phase 2's 25 MiB extraction limit contradicted a live boundary and its 10 MiB download cap had no owner.
  - `toolkit-cache-manager.test.ts` already contains the single-flight case, so that instruction became an extension.
  - `WorkspaceService.openWorkspace` does not exist; the method is `open()` at `workspace-service.ts:745`. `409 STALE_GENERATION` is not a member of `ErrorCodeSchema`. Tier ranking already exists in three places and projection already exists as `composeOwnerToolkitProjection`, so phase 3 now names what `SkillResolver` supersedes instead of implying a greenfield build.
  - Phase 4's new Docker suite was unreachable: `npm run test:docker` is a hardcoded five-file list and the other suites exclude `*.docker.test.ts`.
...
- Re-verified as unchanged and still correct: `docker/executor.Dockerfile` PATH ownership at `/etc/profile.d/harness.sh:36-37` with the `/opt/harness` copies at `:42-48`; `runPrivilegedEphemeralExec` at `workspace-service.ts:1774`; `.agents/skills/cloudharness` and `plugins/cloud-harness/.claude-plugin` both exist; the fixed secret-name list at `verify-compose-boundaries.mjs:80-82` still would not catch a new secret name.
- Still unverifiable inside this repository and recorded as such: the SkillX HTTP API surface, the live skills.sh/SkillX hostnames for the proxy allowlist, and every TypeSafe and Claude Code hook contract.
...
5. **Delivery shape (operator decision).** Resolved: the operator chose to re-verify phases 2-9 and then implement the whole plan on this branch. The advisory checkpoint had recommended reviewed increments instead; that recommendation is recorded but not adopted.
...
All four assumptions below now have external evidence, and each one is load-bearing for at least one phase. What the checks returned replaced what this plan had assumed, and the sections that depended on those assumptions were changed to match rather than left as they were. Where a probe covered only part of an assumption, the uncovered part is stated as still inferred.

1. **SkillX API shape — VERIFIED for search, still inferred for import.** A live probe of `GET https://skillx.sh/api/search?q=pdf` returned `status 200` with `{"results":[{"id","name","slug","description","content"}]}`, and that envelope is now pinned by `apps/runner/test/registry-search.test.ts`. The import path is a different endpoint: it reads one skill through `/api/skills/<slug>` and expects an `instructions` field, which the probe did not exercise. The search envelope is observed; the per-skill envelope remains inferred from the documented CLI behaviour.
2. **`skills` and `skillx` CLI versions — VERIFIED; the offline mirror is not.** `npm view` reports `skills` at **1.7.0** (latest, with a `1.5.23-snapshot.2` snapshot tag) and `skillx-sh` at **0.3.0** (latest), so both versions can be pinned. No mirror is published or recorded anywhere in this tree, so the Phase 7 lifecycle test still reaches the registry rather than a local copy, and that half of the assumption stands.
3. **skills.sh and SkillX hostnames — VERIFIED, and narrower than this plan assumed.** `https://skills.sh` answers `308` to `https://www.skills.sh/` and `https://skillx.sh` answers `200`, so both hostnames resolve as the allowlist assumed. The blast radius is smaller than recorded here: search requests are made by the runner directly rather than through `provisioning-proxy`, so that allowlist does not gate them, and the skills.sh import path resolves its references to GitHub. The allowlist would only matter for an import that called a registry host directly, which neither provider does today.
