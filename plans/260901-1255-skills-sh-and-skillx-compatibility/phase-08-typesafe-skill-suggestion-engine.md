...
- **Redaction and egress safety:**
  - Before egress, run the prompt through the existing redaction primitives (`SecretSnapshotRedactor` and `StreamRedactor` in `apps/runner/src/output-redactor.ts`), then apply pattern redaction for bearer tokens, API-key shapes, and JWT shapes. Name the two value sources explicitly, because the existing path only covers one of them:
    - Workspace and owner secret values come from `getRedactor(workspaceId)` in `apps/runner/src/workspace-service.ts:220-241`, which reads `getSecretSnapshot(workspaceId)` and decrypts each envelope.
    - Provider credentials are **not** in that path. They live in `model_provider_credential_versions` and are decrypted through the gateway export in `apps/runner/src/model-profile-state-repository.ts:721-741`. This phase must either add an explicit, bounded enumeration of those values into the redactor (preferred, with the file added to this phase's scope and a fixture test proving a provider key present in a prompt never reaches the outbound body) or drop the provider-credential claim and record the residual risk. Leaving the claim with no owner is not acceptable.
