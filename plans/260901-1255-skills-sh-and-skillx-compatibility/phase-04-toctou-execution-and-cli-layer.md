...
**Full-lane result, recorded as the contract requires:** `npm run test:docker` reports `Test Files 1 failed | 4 passed | 1 skipped (6)` and `Tests 18 passed | 4 skipped (22)`. The new `apps/runner/test/cli-compatibility-airgap.docker.test.ts` is among the passing files and passes all seven of its cases when run on its own. The one failing suite is `test/integration/agent-isolation.docker.test.ts`, and it fails before its tests run: `scripts/generate-model-gateway-test-fixtures.mjs` calls `generated.stderr.trim()` and that value is undefined on this Windows host, so the TLS fixture generation throws and the suite never starts. That generator is untouched by this branch, so the failure is a pre-existing host limitation rather than a defect in this phase; it is recorded here instead of being worked around, because weakening the lane to hide it would remove coverage that has nothing to do with this work.
