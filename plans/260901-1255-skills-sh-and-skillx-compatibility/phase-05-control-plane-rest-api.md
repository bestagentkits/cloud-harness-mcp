...
**Also done:**
- All twenty-one skills operations now have an explicit `mapDashboardData` branch with an allowlist built from the real Phase 1 columns, plus `operationMessages` entries for the codes each operation can return. Owner identifiers and provider tokens are dropped by construction, a revision diff passes through only when the runner sent a string, bulk results keep per-item outcomes, search keeps provider warnings so a degraded provider stays visible, and a preview always emits its three collections. Covered by `apps/api/test/dashboard-skills-mapper.test.ts` (6 tests).
- The pending list fell from twenty-four entries to three.
- The mapper boundary is now the named `DashboardProjection` instead of `unknown`. Every branch was read and confirmed to return an object, so the type is accurate rather than aspirational, and callers no longer re-narrow.
- API suite green at 34 files / 424 tests; `npm run typecheck` clean.
...
- Runner handlers for the internal operations. `dashboard-control-service.execute()` is a `switch (parsed.operation)` that throws `NOT_FOUND` for any operation without a `case`, so these operations exist in the contract but cannot execute yet. Wiring them needs the answer to a question this phase has not resolved: the twenty skill methods from Phase 1 live on `StateStore`, while the service dispatches through `this.metadata`, `this.secrets()`, and `this.models()`, so the accessor for the skill registry has to be decided before handlers can be written.
- The REST routes themselves, plus their route tests: skills, skill sets, registry sources, imports, revisions, and search, along with owner isolation, the generation `409`, the dependency `409` on delete, and revision diff/restore/fork. `DashboardRoutableOperation` already accepts the skills operations, so none of these routes can be added to the router and forgotten in the mapper.
