...
1. **Tests-First (TDD):**
   - Write a migration test that opens a store whose `schema_meta.version` is 9, containing only the v9 tables, runs the migration, and asserts that every new table exists, `version` is 10, and the same database reopened at version 10 is a no-op. This is the regression test for the ladder owning the DDL rather than the bootstrap block.
...
3. **Database Schema Migration:**
   - Add the skill management, snapshot, and `skill_import_jobs` DDL inside a new `if (version === 9)` ladder step in `apps/runner/src/principal-store.ts` that ends with `UPDATE schema_meta SET version = 10`, following the existing v9 step that created `model_provider_credential_versions`. Do **not** add the DDL to the `StateStore` constructor's unconditional `CREATE TABLE IF NOT EXISTS` block: that block runs on every open before `migratePrincipalSchema(this.database)`, so tables added there would appear on a version 9 database without any version transition and the ladder would not own them.
   - Own the schema version bump in `apps/runner/src/principal-store.ts`, which is where the live migration ladder lives and currently ends at `UPDATE schema_meta SET version = 9`. The new migration takes the store to version 10; `state-store.ts` only seeds version 1 and must not be treated as the ladder owner.
...
- [ ] The new tables land as schema version 10, with the DDL owned by the `if (version === 9)` ladder step in `apps/runner/src/principal-store.ts` rather than the `StateStore` bootstrap block, and the composite foreign keys use the pre-existing `workspaces_owner_id_id` index rather than a duplicate.
