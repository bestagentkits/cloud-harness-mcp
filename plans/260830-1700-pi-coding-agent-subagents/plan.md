...
## Phases
1. [Phase 01: Contracts & Tool Schemas](phase-01-contracts-and-schemas.md) — Define `AgentId`, tool inputs/outputs, 6 `agent_*` schemas, stdio JSONL tool proxy definitions, and Model Gateway control contracts in `packages/contracts`.
...
5. [Phase 05: Tests, Verification, Documentation & Release Readiness](phase-05-tests-and-verification.md) — Unit tests across packages, SQLite transaction/fencing tests, Docker E2E fake TLS provider test, 32-way concurrency, crash injection matrix, zero-residual checks, internal docs (`docs/`), documentation site (`docs-site/`), agent skill & plugin sync (`npm run plugin:sync`), and root verification gates.
