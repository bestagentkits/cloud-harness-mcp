...
export function downgradeStateSchemaToV7(database: DatabaseSync, allowDataLoss = false): void {
  ...
  if (!allowDataLoss) {
    ...
    if (cacheCount > 0 || wsCount > 0) {
      throw new Error('cannot downgrade state schema to v7: toolkit tables contain active records (export or discard required)');
    }
  }
  transaction(database, () => {
    database.exec(`
...
export function downgradeStateSchemaToV5(database: DatabaseSync, allowDataLoss = false): void {
  ...
  if (version === 7) {
    downgradeStateSchemaToV6(database, allowDataLoss);
    version = 6;
  }
