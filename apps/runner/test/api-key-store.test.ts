...
describe('ApiKeyStore', () => {
  ...
  it('downgrades only API-key state and can migrate forward again', () => {
    ...
    expect((value.metadata.database.prepare('SELECT version FROM metadata_schema_meta').get() as { version: number }).version).toBe(6);
    value.metadata.close(); value.state.close();
  });
  ...
  it('fails closed for future, conflicting, and invalid downgrade schema states', () => {
    ...
    expect(() => downgradeMetadataSchemaToV1(future.metadata.database)).toThrow('metadata schema must be version 2, 3, 4, 5, or 6 before downgrade');
